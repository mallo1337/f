...
if __name__ == "__main__":
    asyncio.run(main())from dotenv import load_dotenv
from database import Database
...
MODES = {
    'Pistol DM': {'key': 'pistol_dm', 'weapons': 'G22, USP, P350, Berettas, TEC-9, F/S, Desert Eagle'},
    'Rifle DM': {'key': 'rifle_dm', 'weapons': 'FN FAL, FAMAS, VAL, M4, M4A1, AKR, AKR12, M16'},
    'Sniper DM': {'key': 'sniper_dm', 'weapons': 'M40, Mallard, M110, AWM'},
    'HeadShot DM': {'key': 'hs_dm', 'weapons': 'Доступны все оружия!'},
    '1HP DM': {'key': '1hp_dm', 'weapons': 'Доступны все оружия!'},
    'SMGs DM': {'key': 'smgs_dm', 'weapons': 'MAC10, UMP45, MP5, MP7, Akimbo Uzi, P90'}
}

MAPS = ['🏜 SandStone', '🏘 Province', '🧱 Rust', '🏭 Zone 7', '🌸 Hanami', '🌊 Breeze', '🏝 Dune']
TIMES = ['5 минут', '10 минут', '15 минут', '20 минут', '25 минут', '30 минут']
DAMAGE_TYPES = ['только в голову', 'по всему телу']
REGIONS = ['Россия, Франция']
...
def is_valid_game_id(game_id):
    """Проверка валидности игрового ID (только цифры, 2-13 символов)"""
    try:
        return game_id.isdigit() and 2 <= len(game_id) <= 13
    except:
        return False
    ...
    try:
        ...
        for player_id, player_nickname in players:
            ...
            if player_data:
                rating = player_data[5]
                level = get_player_level(rating)
                topic_text += f"{level} {player_nickname}\n<code>{player_id}</code>\n"
...
async def notify_player_about_processing(user_id, lobby_unique_id, kills, deaths):
    try:
        if not db.is_user_registered(user_id):
            logger.warning(f"User {user_id} not registered, skipping notification")
            return
...
def format_lobby_info(lobby_info, players, creator_first_name=None):
    ...
    try:
        ...
        for i, (player_id, player_nickname) in enumerate(players, 1):
            lobby_text += f"{i}. {player_nickname}\n"
...
async def show_profile_from_message(message: types.Message):
    ...
    user_id_db, username, nickname, game_id, registration_date, rating, matches_played, kills, deaths = player_data
    
    today = datetime.now()
    start_of_week = today - timedelta(days=today.weekday())
    start_of_week_str = start_of_week.strftime("%Y-%m-%d")
    
    position = db.get_player_all_time_position(user_id)
    kd_ratio = kills / deaths if deaths > 0 else kills
    level = get_player_level(rating)
    
    username_display = nickname
...
async def cmd_update_stats(message: types.Message):
    ...
    if len(args) == 4:
        try:
            ...
            if success:
                await message.answer(f"✅ Статистика для игрока {tg_id} в лобби #{lobby_unique_id} обновлена!")
                await notify_player_about_processing(tg_id, lobby_unique_id, kills, deaths)
            else:
                await message.answer("❌ Ошибка при обновлении статистики.")
...
async def start_create_lobby(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await cleanup_user_messages(callback_query.from_user.id)
    await state.set_state(CreateLobbyStates.waiting_for_players)
    await send_message_with_image(callback_query.from_user.id, "🎮 Создание лобби\n\nВведите количество игроков для лобби (от 8 до 16):", None, get_cancel_keyboard())
    await callback_query.answer()
...
async def process_lobby_players(message: types.Message, state: FSMContext):
    try:
        ...
        if max_players < 8 or max_players > 16:
            await message.answer("❌ Введите число от 8 до 16")
            return
        ...
        await message.answer("❌ Пожалуйста, введите число от 8 до 16")
...
async def process_screenshot(message: types.Message, state: FSMContext):
    ...
    player_data = db.get_player_profile(user_id)
    player_nickname = player_data[2] if player_data else "Неизвестно"
...
async def process_nickname(message: types.Message, state: FSMContext):
    ...
    await send_message_with_image(
        user_id,
        "🆔 Введите ваш игровой ID (только цифры, от 2 до 13 символов):",
        None,
        get_cancel_keyboard()
    )
...
async def process_game_id(message: types.Message, state: FSMContext):
    ...
    if not is_valid_game_id(game_id):
        await message.answer("❌ Игровой ID должен содержать только цифры (от 2 до 13 символов)")
        return
...
async def show_weekly_top(callback_query: types.CallbackQuery):
    ...
    if not top_players:
        ...
        for i, player_data in enumerate(top_players):
            ...
            top_text += f"{medal} {nickname} | {level}\n🏆 {rating} | K/D: {kd_ratio:.2f} | 🎮 {matches}\n\n"
